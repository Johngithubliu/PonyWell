...
static s32 ATRsp_IMSI_Handler(char* line, u32 len, void* param)
{
	...
	pHead = Ql_RIL_FindString(line, len, "460");//fail
    if (pHead)
    {
        char* p1 = NULL;
        char* p2 = NULL;
...
s32 RIL_GetIMSI(char* imsi)
{
    ...
    return Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATRsp_IMSI_Handler,(void*)imsi, 300);
}
//iccid
static s32 ATRsp_ICCID_Handler(char* line, u32 len, void* param)
{
	...
	pHead = Ql_RIL_FindString(line, len, "8986");//fail
    if (pHead)
    {
        char* p1 = NULL;
        char* p2 = NULL;
...
static s32 ATRsp_CSQ_Handler(char* line, u32 len, void* param)
{
	...
	pHead = Ql_RIL_FindString(line, len, "+CSQ:");//fail
    if (pHead)
    {
        char* p1 = NULL;
        char* p2 = NULL;
...
s32 RIL_GetCSQ(char* csq)
{
    ...
    return Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATRsp_CSQ_Handler,(void*)csq, 300);

}
//-------
static s32 ATRsp_NTP_Handler(char* line, u32 len, void* param)
{
    char* pHead = NULL;
    pHead = Ql_RIL_FindLine(line, len, "OK"); // find <CR><LF>OK<CR><LF>, <CR>OK<CR>，<LF>OK<LF>
    if (pHead)
    ...
    pHead = Ql_RIL_FindString(line, len, "+QNTP:");//fail
    if (pHead)
    {
        char* p1 = NULL;
        char* p2 = NULL;
...
s32 RIL_GetNtpTime(char* timestr)
{
    char strAT[] = "AT+QNTP=1,\"ntp5.aliyun.com\"\r\n";
    if (NULL == timestr)
    {
        return RIL_AT_INVALID_PARAM;
    }
    return Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATRsp_NTP_Handler,(void*)timestr, 300);
    ...
    while (p != NULL && *p != '\0' && param_num < param_max_num) {
        ...
        if (*p == '\"') {
            ...
            *(param + len) = '\0';
        	...
        	if ( param_num == 7 )  //ready to parse "value" ---> param_list[7]
        	{
				if ( *p == '{' )  //json格式数据解析流程
				{
					strncpy(param, p, temp_value_len);
					*(param + temp_value_len) = '\0';
					p += temp_value_len;
					p++;
				}
				else
			...
			else
			{
				...
				if ( p_temp == NULL )  //last
				{
					u32 last_len = p_end - p;
					memcpy(param, p, p_end - p);
					param_len = strlen(param);
					for (i = param_len - 1; i >= 0; i--) {
			            if (*(param + i) == ' ' || *(param + i) == '\t') {
			                *(param + i) = '\0';
		...
		if ( param_num == 7 )
		{
			temp_value_len = atoi(param_list[6]);
		}
        param = param + strlen(param) + 1;
    }
    return param_num;
